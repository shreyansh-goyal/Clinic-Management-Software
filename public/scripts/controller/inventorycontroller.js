app.controller("inventorycontroller",function($scope,storefactory){
    ...
    //This is the function to add the medicene to the bill
     $scope.addToBill=(mediceneObj)=>{
        var obj={};
         fetch(`https://ec2-13-232-8-74.ap-south-1.compute.amazonaws.com:1234/getunits?name=${mediceneObj.Medicene.name}`).then(data=>{
            data.json().then(data=>{
    ...
    $scope.generateInvoice=()=>{
        fetch("https://ec2-13-232-8-74.ap-south-1.compute.amazonaws.com:1234/checkUnits");
        console.log($scope.billArray);
            ...
            fetch("https://ec2-13-232-8-74.ap-south-1.compute.amazonaws.com:1234/updateMedcieneUnits",{
                method: "POST", 
                mode: "cors",
                cache: "no-cache",
